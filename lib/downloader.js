...
export async function tiktok(url) {
    try {
        let { data } = await axios.get('https://api.roidev.my.id/api/tiktok', {
            params: {
                url
            }
        })
        ...
        return {
            ...
            video: data.result.video[1].url || data.result.video[0].url,
...
export async function threadsDl(url) {
    try {
        let { data } = await axios.get('https://api.roidev.my.id/api/threads', {
            params: {
                url
            }
        })
        // return data
        return { title: data.data.title || 'Kanata V3', author: data.data.author, downloadUrl: data.data.downloadUrl }
    } catch (error) {
        return error
    }
}
// console.log(await tiktok('https://vt.tiktok.com/ZSjWP7x83/'))
console.log(await threads('https://www.threads.net/@abdulrahimroni/post/DG6NL3dtKO5'))
export async function fb(url) {
    try {
        let { data } = await axios.get('https://api.roidev.my.id/api/facebook', {
            params: {
                url
            }
        })
...
export async function capcut(url) {
    try {
        let { data } = await axios.get('https://api.roidev.my.id/api/capcut', {
            params: {
                url
            }
        })
