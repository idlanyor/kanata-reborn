...
export async function menuCard(username, handle, today, avatarUrl) {
    ...
    try {
        ...
        ctx.fillText('Creator: Roy', canvas.width - 50, 225);
        ctx.fillText('Prefix: multi-prefix', canvas.width - 50, 255);
        ...
        ctx.fillText('• Productivity Tools', 50, 225);
        ctx.fillText('• Game & Fun', 50, 255);

        // Tambahkan dekorasi di sisi kiri bawah
        ctx.font = 'bold 18px Delius';
        ctx.fillStyle = '#FFD700';
        ctx.fillText('Ketik .menu untuk daftar fitur', 50, canvas.height - 70);
